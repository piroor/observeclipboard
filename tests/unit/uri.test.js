...
function test_matchURIRegExp()
{
	...
	function assert_matchURIRegExp(aFullWidth, aRelative)
	{
		...
		assertURIMatch(halfWidth_relative_URI, halfWidth_relative_URI, false);
		assertURIMatch(fullWidth_relative_URI, fullWidth_relative_URI, false);
		assertURIMatch('test '+halfWidth_relative_URI+' text', halfWidth_relative_URI, false);
		assertURIMatch('test '+fullWidth_relative_URI+' text', fullWidth_relative_URI, false);
		assertURIMatch('日本語'+halfWidth_relative_URI+'日本語', halfWidth_relative_URI, false);
		assertURIMatch('日本語'+fullWidth_relative_URI+'日本語', fullWidth_relative_URI, false);
...
[
	...
	{ base : 'www.example.com/directory' }
].forEach(function(aPattern) {
	parens.forEach(function(aParen) {
		parenPatterns.push({ input     : aParen[0] + aPattern.base + aParen[1],
		                     sanitized : aPattern.base });
		parenPatterns.push({ input     : aParen[0] + 'http://' + aPattern.base + '/' + aParen[1],
		                     sanitized : 'http://' + aPattern.base + '/' });
		parenPatterns.push({ input     : aParen[0] + 'http://' + aPattern.base + '/index.html' + aParen[1],
